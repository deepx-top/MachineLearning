# -*- coding:utf-8 -*-

import numpy as np
import matplotlib.pyplot as plt

from keras.models import Sequential
from keras.layers import Dense, Flatten
from keras.layers.normalization import BatchNormalization
from keras.callbacks import History
from keras import regularizers
from keras.optimizers import SGD,RMSprop,Adadelta,Adagrad

def figures(history, figure_name="plots"):
    """ method to visualize accuracies and loss vs epoch for training as well as testind data
        Argumets: history     = an instance returned by model.fit method
                  figure_name = a string representing file name to plots. By default it is set to "plots"
       Usage: hist = model.fit(X,y)
       figures(hist) """
    if isinstance(history, History):
        hist = history.history
        epoch = history.epoch
        acc = hist['acc']
        loss = hist['loss']
        val_loss = hist['val_loss']
        val_acc = hist['val_acc']
        plt.figure(1)

        plt.subplot(2, 2, 1)
        plt.plot(epoch, acc)
        plt.title("Training accuracy vs Epoch")
        plt.xlabel("Epoch")
        plt.ylabel("Accuracy")

        plt.subplot(2, 2, 2)
        plt.plot(epoch, loss)
        plt.title("Training loss vs Epoch")
        plt.xlabel("Epoch")
        plt.ylabel("Loss")

        plt.subplot(2, 2, 3)
        plt.plot(epoch, val_acc)
        plt.title("Validation Acc vs Epoch")
        plt.xlabel("Epoch")
        plt.ylabel("Validation Accuracy")

        plt.subplot(2, 2, 4)
        plt.plot(epoch, val_loss)
        plt.title("Validation loss vs Epoch")
        plt.xlabel("Epoch")
        plt.ylabel("Validation Loss")
        plt.tight_layout()
        plt.savefig(figure_name)
    else:
        print("Input Argument is not an instance of class History")


data = np.loadtxt('./data1.txt', delimiter=',')
np.random.shuffle(data)
x, y = data[:, 0:2], data[:, 2]
# x = np.c_[np.ones((x.shape[0], 1)), x]

# x = (x - x.min()) / (x.max() - x.min())
# y = (y - y.min()) / (y.max() - y.min())
# x = normalize(x)
# y = normalize(y)
# plt.scatter(x, y)
x_train, y_train = x[:80, :], y[:80]
x_test, y_test = x[81:, ], y[81:]

model = Sequential()
# model.add(Dense(20,   activation='sigmoid'))
# model.add(Dense(20, activation='sigmoid'))
# model.add(Dense(10, activation='sigmoid'))
model.add(Dense(7,kernel_regularizer=regularizers.l1(0.01), input_shape=(2,), activation='sigmoid'))
model.add(Dense(1, activation='sigmoid'))
# model.add(Flatten())
rms=RMSprop(lr=0.001)
model.compile(optimizer=rms,
              loss='binary_crossentropy',
              metrics=['accuracy'])
# print(x_train.shape,y_train.shape)
# x_train, y_train = np.random.random(
#     (80, 2)), np.random.randint(2, size=(80, 1))
# print(x_train.shape,y_train.shape)
hist = model.fit(x_train, y_train, batch_size=10, epochs=3000,
                 shuffle=True, verbose=2, validation_split=0.3)

score = model.evaluate(x_test, y_test,  verbose=1)
print(score)
y_pre = model.predict(x_test, batch_size=5)
print(np.c_[y_pre,y_test])
# plt.plot(x_test, y_pre, 'k--', lw=2)
# plt.plot(x_test, y_test, 'rx')
# plt.show()
figures(hist)
